...
export class ModificarPage implements OnInit {
  ...
  constructor(
    ...
    if (navigation && navigation.extras.state) {
      ...
      if (reserva) {
        ...
        this.fecha = new Date(reserva.fecha); 
        this.noches = reserva.noches; 
        this.idhabitacion = reserva.idhabitacion; // Cargar el idhabitacion
      }
    }
  }
  ...
  async reservar() {
    ...
    if (fechasReservadas.length > 0) {
      ...
      await this.bd.modificarReserva(this.idreserva,fechaSinHora.toISOString().split('T')[0],this.noches);
      const alert = await this.alertController.create({
        header: 'Modificar',
  ...
  async cargarFechasReservadas() {
    ...
    console.log('Fechas desactivadas:', this.fechasDesactivadas); // Mensaje de depuración
  }
